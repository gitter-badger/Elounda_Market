...
FROM ESFIItemPeriodics AS ESFIItemPeriodics
     ...
     WHERE
       ...
       AND (FK_ESFIItemPeriodics_ESFIItem.fItemSubcategoryCode in ('Κοπέλι', 'Chocodream', 'Creta', 'Cretan Herbs', 'La Follie', 'Κερούλης', 'Κοπέλι', 'Κρητικος Φουρνος', 'Περυσινάκη'))
...
where
 (ESFIItem.fItemSubcategoryCode in ('Κοπέλι', 'Chocodream', 'Creta', 'Cretan Herbs', 'La Follie', 'Κερούλης', 'Κρητικος Φουρνος', 'Περυσινάκη'))
...
with pd.ExcelWriter(file_path, engine='xlsxwriter') as writer:
    ...
    # MERGE CELLS
    worksheet.merge_range("M12:R12", 'ΤΕΛΕΥΑΤΑΙΑ ΕΝΗΜΕΡΩΣΗ', set_color)
    worksheet.merge_range("M13:R13", 'ΗΜΕΡΟΜΗΝΙΑ: {}  '.format(datetime.now().strftime("%d/%m/%Y")), set_color)
    worksheet.merge_range("M14:R14", 'ΩΡΑ: {}  '.format(datetime.now().strftime("%H:%M:%S")), set_color)

    # Φτιάχνω το Γράφημα
    chart02 = workbook.add_chart({'type': 'column'})
    chart02.add_series({'categories': '=ΚΕΡΟΥΛΗΣ!$B$2:$B$10',
                        'values': '=ΚΕΡΟΥΛΗΣ!$C$2:$C$10',
                        'name': 'ΕΤΗΣΙΟΣ ΤΖΙΡΟΣ'
                        })
