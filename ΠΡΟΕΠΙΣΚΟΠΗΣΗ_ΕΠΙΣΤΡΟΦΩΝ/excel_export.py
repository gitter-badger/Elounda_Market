...
def export(file_path, answer_01, answer_02, katastima):
    with pd.ExcelWriter(file_path, engine='xlsxwriter', datetime_format='dd/mm/yy') as writer:
        ...
        worksheet.set_column('D:D', 12, center)
        worksheet.set_column('E:E', 12, center)
        worksheet.set_column('F:F', 18, center)
        worksheet.set_column('G:G', 40, center)
        worksheet.set_column('H:H', 10, center)


        # MERGE CELLS
        worksheet.merge_range("A1:D1", 'ΠΡΟΣΩΡΙΝΗ ΕΓΓΡΑΦΗ: ΚΟΥΤΟΥΛΑΚΗΣ Μ. ΑΤΣΑΛΗΣ Α. Ο.Ε.', set_color)
